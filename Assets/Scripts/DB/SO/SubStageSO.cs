...
public class SubStageSO : MonoSO<SubStageData>
{
    ...
    public override void SetData(Dictionary<int, SubStageData> DB)
    {
        for (int i = 0; i < SubStageList.Count; i++)                // 엑셀 데이터 개수만큼 반복문 반복, 데이터 가져와서 딕셔너리에 넣어주는 반복문
        {
            var data = SubStageList[i];                             // data : 엑셀 데이터 중 i번째 데이터.
